<script lang="ts">
  ...
  function clampScroll() {
    ...
    const max = Math.max(
      0,
      console2d.getContentHeight() - console2d.getViewportHeight()
    );
  ...
  function handlePointerMove(e: PointerEvent) {
    ...
    if (console2d.getContentHeight() <= console2d.getViewportHeight()) {
      return;
    }
  ...
  function handlePointerUp(e: PointerEvent) {
    ...
    safeWrap(() => canvasEl.releasePointerCapture(e.pointerId));

    // Treat as click if it didn't turn into a drag.
    if (!dragMoved) {
  ...
  onMount(() => {
    ...
    (async () => {
      const [err, THREE] = await safeWrapAsync(() => import("three"));
      if (unmounted) return;
      if (err) {
        console.error(err);
        onfail?.();
      ...
      const [crtErr, renderer] = await safeWrapAsync(async () => {
        return createCrtRenderer(THREE, {
          canvas: canvasEl,
          sourceCanvas,
          config,
        });
