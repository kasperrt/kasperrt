...
import { getShellReply } from "./shell";
...
export function initTerminal(signal: AbortSignal, desktop: Desktop) {
  ...
  function renderLine(line: Line) {
    ...
    row.className = "mb-2 whitespace-pre-wrap";
    if (line.command) {
    ...
    if (line.href) {
      ...
      link.className = "text-blue-300 visited:text-blue-300";
  ...
  function updateTime() {
    if (terminal?.hidden || document.hidden) {
      return;
    }
    const now = new Date();
